...
/** Implements the "/" operation. Divides numbers, shortens strings. */
public final class Div extends Instruction.Op {
  public Div() { super("/", 2, 1); }

  /** Pops two values and pushes a result. The calculation depends on the types
   * of the values popped:
   * <ul>
   *  <li>If both operands are numbers, the result is the bottom-most divided by
   *      the top-most.
   *  <li>If the bottom-most is a string and the top-most an integer, the result
   *      is formed by keeping the specified number of characters from the
   *      right-hand end of the string.
   *  <li>If the top-most is a string and the bottom-most an integer, the result
   *      is formed by keeping the specified number of characters from the
    ...
    switch (x.type + (y.type<<3)) {
      ...
      case Value.TYPE_NUM + (Value.TYPE_STR<<3): {
        ...
        if (xNum<0 || xNum>yStr.length()) throw new CrazonRuntimeException(
          "Cannot keep "+x+" characters from "+y+": index out of range",
          state
        );
        final String ans = yStr.substring(0, xNum);
      ...
      case Value.TYPE_STR + (Value.TYPE_NUM<<3): {
        ...
        if (yNum<0 || yNum>xStr.length()) throw new CrazonRuntimeException(
          "Cannot keep "+y+" characters from "+x+": index out of range",
          state
        );
        final String ans = xStr.substring(xStr.length()-yNum);
        state.push(new Value.Str(ans));
        return;
      }
