...
public final class Xor extends Instruction.Op {
  public Xor() { super("XOR", 2, 1); }

  /** Pops two booleans 'x' and 'y' and pushes a their logical XOR. */
  ...
  throws CrazonRuntimeException {
    ...
    try {
      ...
      throw new CrazonRuntimeException(
        "Cannot apply XOR to "+x+" and "+y+"; two booleans are required",
        state
      );
    }
  }
  ...
  public static void test(Value x, Value y) throws CrazonRuntimeException {
    ...
    state.push(x);
    state.push(y);
    new Xor().execute(state);
    System.out.println(x+" XOR "+y+" = "+state.pop());
  }
