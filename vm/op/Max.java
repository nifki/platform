...
/** Implements the "MAX" operation in terms of 'compareTo()'. */
public final class Max extends Instruction.Op {
  public Max() { super("MAX", 2, 1); }
  ...
  public static void test(Value x, Value y) throws CrazonRuntimeException {
    ...
    state.push(x);
    state.push(y);
    new Max().execute(state);
    System.out.println(x+" MAX "+y+" = "+state.pop());
  }
